"""
RandomC (Random with Cooperative) Task Scheduling Implementation
This module implements a modified Random algorithm that prioritizes cooperative resource sharing between fog and cloud nodes.
It extends the basic Random approach with cooperative scheduling and resource optimization mechanisms.
"""
...
# Initialize logger for RandomC events
randomc_logger = setup_logger('randomc', 'randomc.log', sub_directory='algorithms')
...
def process_fcfs(tasks):
    """
    Process tasks using RandomC algorithm with cooperative fog-cloud approach
    
    This function implements the main RandomC scheduling logic:
    ...
    if not tasks:
        ...
        randomc_logger.warning("No tasks to process.")
        return
        
    print("\n=== RandomC Hybrid Processing (Fog-Cloud) ===")
    print(f"Total Tasks to Process: {len(tasks)}")
    randomc_logger.info(f"Total Tasks to Process: {len(tasks)}")
    
    # Initialize and log fog nodes
    fog_nodes = get_all_fog_nodes()
    for name, node in fog_nodes.items():
        randomc_logger.info(f"Fog node created: {name} (MIPS={node.mips}, RAM={node.memory}, BW={node.bandwidth}, Location={node.location})")
    
    # Initialize and log cloud nodes
    cloud_nodes = get_all_cloud_nodes()
    for name, node in cloud_nodes.items():
        randomc_logger.info(f"Cloud node created: {name} (MIPS={node.mips}, RAM={node.memory}, BW={node.bandwidth}, Location={node.location})")
    ...
    def task_completion_callback(node_name, completion_info):
        ...
        randomc_logger.info(f"\nTask Completed: {task_name}")
        randomc_logger.info(f"  Completed at: {node_name}")
        randomc_logger.info(f"  Transmission Time: {completion_info.get('transmission_time', 0):.2f}s")
        randomc_logger.info(f"  Queue Time: {queue_time:.2f}s")
        randomc_logger.info(f"  Processing Time: {completion_info.get('processing_time', 0):.2f}s")
        randomc_logger.info(f"  Storage Used: {task.get('Storage', 0):.2f}GB")
        randomc_logger.info(f"  Total Time: {task_completion_info[task_name]['total_time']:.2f}s")
        if power_info:
            randomc_logger.info(f"  Power Consumption: {power_info.get('total_energy_wh', 0):.6f} Wh")
            randomc_logger.info(f"  Average Power: {power_info.get('avg_power_watts', 0):.2f} W")
        randomc_logger.info("  " + "-" * 30)
    ...
    for i, task in enumerate(sorted_tasks, 1):
        ...
        randomc_logger.info("Fog Node Distances:")
        for fog_name, distance in sorted_fog_nodes:
            randomc_logger.info(f"  {fog_name}: {distance:.2f} km")
        randomc_logger.info("Cloud Node Distances:")
        for cloud_name, distance in sorted_cloud_nodes:
            randomc_logger.info(f"  {cloud_name}: {distance:.2f} km")
        ...
        if not is_cloud_task:
            ...
            if fog_node:
                ...
                if fog_node.can_handle_task(task):
                    ...
                    if success:
                        ...
                        randomc_logger.info(f"Task successfully assigned to fog node {first_fog_name}")
                        ...
                        randomc_logger.info(f"Fog node {first_fog_name} cannot handle task - trying second fog node")
                        print(f"\nFog node {first_fog_name} cannot handle task due to resource constraints")
                else:
                    randomc_logger.info(f"Fog node {first_fog_name} does not have sufficient resources")
                ...
                if not task_assigned and len(sorted_fog_nodes) > 1:
                    ...
                    if fog_node:
                        randomc_logger.info(f"Attempting to assign task to fog node {second_fog_name} ({second_fog_distance:.2f} km)")
                        # Check all resources before assignment
                        if fog_node.can_handle_task(task):
                            success, processing_time = fog_node.assign_task(task)
                            if success:
                                # Task successfully assigned to second fog node
                                randomc_logger.info(f"Task successfully assigned to fog node {second_fog_name}")
                                ...
                                randomc_logger.info(f"Second fog node {second_fog_name} also cannot handle task - attempting cloud fallback")
                                print(f"\nSecond fog node {second_fog_name} cannot handle task - attempting cloud fallback")
                        else:
                            randomc_logger.info(f"Second fog node {second_fog_name} does not have sufficient resources")
                            print(f"\nSecond fog node {second_fog_name} does not have sufficient resources")
                
                # Try cloud nodes if both fog nodes failed
                if not task_assigned:
                    randomc_logger.info("Attempting cloud fallback")
                    ...
                    for cloud_name, cloud_distance in sorted_cloud_nodes:
                        ...
                        if cloud_node:
                            randomc_logger.info(f"Attempting to assign task to cloud node {cloud_name} ({cloud_distance:.2f} km)")
                            # Check all resources before assignment
                            if cloud_node.can_handle_task(task):
                                ...
                                if success:
                                    ...
                                    randomc_logger.info(f"Task successfully assigned to cloud node {cloud_name}")
                                    ...
                                    randomc_logger.info(f"Cloud node {cloud_name} cannot handle task immediately - queuing")
                                ...
                                randomc_logger.info(f"Cloud node {cloud_name} does not have sufficient resources")
            ...
            for cloud_name, distance in sorted_cloud_nodes:
                cloud_node = get_cloud_node(cloud_name)
                if cloud_node:
                    randomc_logger.info(f"Attempting to assign task to cloud node {cloud_name} ({distance:.2f} km)")
                    # Check all resources before assignment
                    if cloud_node.can_handle_task(task):
                        success, processing_time = cloud_node.assign_task(task)
                        if success:
        ...
        if not task_assigned:
            randomc_logger.error(f"Failed to assign or queue task {task['Name']}")
    ...
    if queued_tasks_info:
        ...
        for task_name, info in queued_tasks_info.items():
            ...
            randomc_logger.info(f"  Queued at: {info['node']} ({info['node_type']}, distance={info['distance']:.2f} km)")
            randomc_logger.info(f"  Queue Position: {info['queue_position']}")
            randomc_logger.info(f"  Task Size: {info['task_size']} GB")
            randomc_logger.info(f"  Required Resources:")
            randomc_logger.info(f"    MIPS: {info['required_mips']}")
            randomc_logger.info(f"    RAM: {info['required_ram']}")
            randomc_logger.info(f"    Bandwidth: {info['required_bw']}")
            randomc_logger.info(f"    Storage: {info['required_storage']}GB")
    ...
    if task_completion_info:
        ...
        randomc_logger.info("\n=== Overall Statistics ===")
        randomc_logger.info(f"Total Tasks: {len(task_completion_info)}")
        randomc_logger.info(f"  Cloud Tasks: {len(cloud_tasks)}")
        randomc_logger.info(f"  Fog Tasks: {len(fog_tasks)}")
        randomc_logger.info("\nWorkload:")
        randomc_logger.info(f"  Total System Workload: {total_workload:.2f} MIPS-seconds")
        randomc_logger.info(f"  Average Workload per Task: {avg_workload:.2f} MIPS-seconds")
        randomc_logger.info("\nResource Usage:")
        randomc_logger.info(f"  Total Storage Used: {total_storage:.2f} GB")
        randomc_logger.info(f"  Total Data Size: {total_size:.2f} MI")
        randomc_logger.info(f"  Total Bandwidth Used: {total_bandwidth:.2f} Mbps")
        randomc_logger.info(f"  Average Storage per Task: {avg_storage:.2f} GB")
        randomc_logger.info(f"  Average Size per Task: {avg_size:.2f} MI")
        randomc_logger.info(f"  Average Bandwidth per Task: {avg_bandwidth:.2f} Mbps")
        randomc_logger.info("\nPower Consumption:")
        randomc_logger.info(f"  Total Energy Consumed: {total_energy_wh:.6f} Wh")
        randomc_logger.info(f"  Total Power Used: {total_power_watts:.2f} W")
        randomc_logger.info(f"  Average Energy per Task: {avg_energy:.6f} Wh")
        randomc_logger.info(f"  Average Power per Task: {avg_power:.2f} W")
        randomc_logger.info("\nTransmission Time:")
        randomc_logger.info(f"  Total: {total_transmission*1000:.6f}ms")
        randomc_logger.info(f"  Average: {avg_transmission*1000:.6f}ms")
        randomc_logger.info("\nProcessing Time:")
        randomc_logger.info(f"  Total: {total_processing*1000:.6f}ms")
        randomc_logger.info(f"  Average: {avg_processing*1000:.6f}ms")
        randomc_logger.info("\nQueue Time:")
        randomc_logger.info(f"  Total: {total_queue*1000:.6f}ms")
        randomc_logger.info(f"  Average: {avg_queue*1000:.6f}ms")
        randomc_logger.info("\nTotal Time:")
        randomc_logger.info(f"  Total: {total_time*1000:.6f}ms")
        randomc_logger.info(f"  Average: {avg_total*1000:.6f}ms")
        
        # Log Fog Statistics
        if fog_tasks:
            randomc_logger.info("\n=== Overall Fog Statistics ===")
            randomc_logger.info(f"Total Fog Tasks: {len(fog_tasks)}")
            randomc_logger.info("\nWorkload:")
            randomc_logger.info(f"  Total Fog Workload: {fog_workload:.2f} MIPS-seconds")
            randomc_logger.info(f"  Average Workload per Fog Task: {avg_fog_workload:.2f} MIPS-seconds")
            randomc_logger.info("\nResource Usage:")
            randomc_logger.info(f"  Total Fog Storage: {fog_storage:.2f} GB")
            randomc_logger.info(f"  Total Fog Data Size: {fog_size:.2f} MI")
            randomc_logger.info(f"  Total Fog Bandwidth: {fog_bandwidth:.2f} Mbps")
            randomc_logger.info(f"  Average Storage per Fog Task: {avg_fog_storage:.2f} GB")
            randomc_logger.info(f"  Average Size per Fog Task: {avg_fog_size:.2f} MI")
            randomc_logger.info(f"  Average Bandwidth per Fog Task: {avg_fog_bandwidth:.2f} Mbps")
            randomc_logger.info("\nPower Consumption:")
            randomc_logger.info(f"  Total Fog Energy Consumed: {fog_energy:.6f} Wh")
            randomc_logger.info(f"  Total Fog Power Used: {fog_power:.2f} W")
            randomc_logger.info(f"  Average Energy per Fog Task: {avg_fog_energy:.6f} Wh")
            randomc_logger.info(f"  Average Power per Fog Task: {avg_fog_power:.2f} W")
            randomc_logger.info("\nPer Node Statistics:")
            for node_name in fog_node_workloads:
                randomc_logger.info(f"\n  {node_name}:")
                randomc_logger.info(f"    Workload: {fog_node_workloads[node_name]:.2f} MIPS-seconds")
                randomc_logger.info(f"    Storage Used: {fog_node_storage[node_name]:.2f} GB")
                randomc_logger.info(f"    Data Size: {fog_node_size[node_name]:.2f} MI")
                randomc_logger.info(f"    Bandwidth Used: {fog_node_bandwidth[node_name]:.2f} Mbps")
                randomc_logger.info(f"    Energy Consumed: {fog_node_energy[node_name]:.6f} Wh")
                randomc_logger.info(f"    Power Used: {fog_node_power[node_name]:.2f} W")
            randomc_logger.info("\nTransmission Time:")
            randomc_logger.info(f"  Total: {fog_transmission*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_fog_transmission*1000:.6f}ms")
            randomc_logger.info("\nProcessing Time:")
            randomc_logger.info(f"  Total: {fog_processing*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_fog_processing*1000:.6f}ms")
            randomc_logger.info("\nQueue Time:")
            randomc_logger.info(f"  Total: {fog_queue*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_fog_queue*1000:.6f}ms")
            randomc_logger.info("\nTotal Time:")
            randomc_logger.info(f"  Total: {fog_total*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_fog_total*1000:.6f}ms")
        
        # Log Cloud Statistics
        if cloud_tasks:
            randomc_logger.info("\n=== Overall Cloud Statistics ===")
            randomc_logger.info(f"Total Cloud Tasks: {len(cloud_tasks)}")
            randomc_logger.info("\nWorkload:")
            randomc_logger.info(f"  Total Cloud Workload: {cloud_workload:.2f} MIPS-seconds")
            randomc_logger.info(f"  Average Workload per Cloud Task: {avg_cloud_workload:.2f} MIPS-seconds")
            randomc_logger.info("\nResource Usage:")
            randomc_logger.info(f"  Total Cloud Storage: {cloud_storage:.2f} GB")
            randomc_logger.info(f"  Total Cloud Data Size: {cloud_size:.2f} MI")
            randomc_logger.info(f"  Total Cloud Bandwidth: {cloud_bandwidth:.2f} Mbps")
            randomc_logger.info(f"  Average Storage per Cloud Task: {avg_cloud_storage:.2f} GB")
            randomc_logger.info(f"  Average Size per Cloud Task: {avg_cloud_size:.2f} MI")
            randomc_logger.info(f"  Average Bandwidth per Cloud Task: {avg_cloud_bandwidth:.2f} Mbps")
            randomc_logger.info("\nPower Consumption:")
            randomc_logger.info(f"  Total Cloud Energy Consumed: {cloud_energy:.6f} Wh")
            randomc_logger.info(f"  Total Cloud Power Used: {cloud_power:.2f} W")
            randomc_logger.info(f"  Average Energy per Cloud Task: {avg_cloud_energy:.6f} Wh")
            randomc_logger.info(f"  Average Power per Cloud Task: {avg_cloud_power:.2f} W")
            randomc_logger.info("\nPer Node Statistics:")
            for node_name in cloud_node_workloads:
                randomc_logger.info(f"\n  {node_name}:")
                randomc_logger.info(f"    Workload: {cloud_node_workloads[node_name]:.2f} MIPS-seconds")
                randomc_logger.info(f"    Storage Used: {cloud_node_storage[node_name]:.2f} GB")
                randomc_logger.info(f"    Data Size: {cloud_node_size[node_name]:.2f} MI")
                randomc_logger.info(f"    Bandwidth Used: {cloud_node_bandwidth[node_name]:.2f} Mbps")
                randomc_logger.info(f"    Energy Consumed: {cloud_node_energy[node_name]:.6f} Wh")
                randomc_logger.info(f"    Power Used: {cloud_node_power[node_name]:.2f} W")
            randomc_logger.info("\nTransmission Time:")
            randomc_logger.info(f"  Total: {cloud_transmission*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_cloud_transmission*1000:.6f}ms")
            randomc_logger.info("\nProcessing Time:")
            randomc_logger.info(f"  Total: {cloud_processing*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_cloud_processing*1000:.6f}ms")
            randomc_logger.info("\nQueue Time:")
            randomc_logger.info(f"  Total: {cloud_queue*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_cloud_queue*1000:.6f}ms")
            randomc_logger.info("\nTotal Time:")
            randomc_logger.info(f"  Total: {cloud_total*1000:.6f}ms")
            randomc_logger.info(f"  Average: {avg_cloud_total*1000:.6f}ms")
...
if __name__ == "__main__":
    ...
    randomc_logger.info(f"Number of tasks received: {len(tasks)}")
    
    # Process tasks using RandomC algorithm
    if tasks:
        process_fcfs(tasks)
    else:
        print("Error: No tasks were loaded")
        randomc_logger.error("No tasks were loaded.")