"""
FCFSC (First Come First Serve with Cooperative) Task Scheduling with Single-Task Fog Nodes
...
from fog_single import get_fog_node, get_fog_node_status, get_all_fog_nodes
...
fcfsc_logger = setup_logger('fcfsc_single', 'fcfsc_single.log', sub_directory='algorithms')
...
def process_fcfs(tasks):
    ...
    for name, node in cloud_nodes.items():
        fcfsc_logger.info(f"Cloud node created: {name} (MIPS={node.mips}, RAM={node.memory}, BW={node.bandwidth}, Location={node.location})")    # Initialize tracking variables
    task_completion_info = {}  # Store completion details for each task
    deferred_tasks_info = {}   # Track tasks that were deferred (couldn't be immediately processed)
    completed_tasks_count = 0  # Counter for completed tasks
    total_tasks = len(tasks)   # Total number of tasks to process
    task_deferral_times = {}   # Track when tasks were deferred
    ...
    def task_completion_callback(node_name, completion_info):
        ...
        task_name = task['Name']        # Calculate wait time if task was deferred
        wait_time = 0
        if task_name in task_deferral_times:
            wait_time = time.time() - task_deferral_times[task_name]
            del task_deferral_times[task_name]
        ...
        task_completion_info[task_name] = {
            ...
            'total_time': completion_info.get('total_time', 0) + wait_time,
        ...
        fcfsc_logger.info(f"  Wait Time: {wait_time:.2f}s")
        ...
        if task_name in deferred_tasks_info:
            del deferred_tasks_info[task_name]
                        ...
                        if cloud_node:
                            ...
                            if cloud_node.can_handle_task(task):
                                ...
                                if success:
                                    ...
                                    # Node is busy, record as deferred task
                                    fcfsc_logger.info(f"Cloud node {cloud_name} is busy - marking as deferred")
                                    print(f"Task deferred for cloud node {cloud_name} ({cloud_distance:.2f} km)")
                                    # Record deferral time
                                    task_deferral_times[task['Name']] = time.time()
                                    deferred_tasks_info[task['Name']] = {
                                        'node': cloud_name,
                                        'node_type': 'Cloud',
                                        'distance': cloud_distance,
                                        'queue_position': 1,  # Position doesn't apply with single-task nodes
                                        'deferral_time': time.time(),
                                        'task_size': task['Size'],
                ...
                if cloud_node:
                    ...
                    if cloud_node.can_handle_task(task):
                        ...
                        if success:
                            ...
                            task_assigned = True
                            break
                        else:                            # Node is busy, record as deferred task
                            fcfsc_logger.info(f"Cloud node {cloud_name} is busy - marking as deferred")
                            print(f"\nTask deferred for cloud node {cloud_name} ({distance:.2f} km)")
                            # Record deferral time
                            task_deferral_times[task['Name']] = time.time()
                            deferred_tasks_info[task['Name']] = {
                                'node': cloud_name,
    ...
    while completed_tasks_count < total_tasks:
        time.sleep(0.1)  # Small delay to prevent CPU overuse        # Log deferred tasks information
    if deferred_tasks_info:
        fcfsc_logger.info("\n=== Deferred Tasks Summary ===")
        for task_name, info in deferred_tasks_info.items():
            fcfsc_logger.info(f"\nDeferred Task: {task_name}")
            fcfsc_logger.info(f"  Deferred at: {info['node']} ({info['node_type']}, distance={info['distance']:.2f} km)")
            fcfsc_logger.info(f"  Wait Status: Deferred (node busy)")
    ...
    if task_completion_info:
        ...
        total_wait = sum(info.get('wait_time', 0) for info in task_completion_info.values())
        ...
        if cloud_tasks:
            ...
            cloud_wait = sum(info.get('wait_time', 0) for info in cloud_tasks)
            ...
            avg_cloud_wait = cloud_wait / len(cloud_tasks)
        ...
        if fog_tasks:
            ...
            fog_wait = sum(info.get('wait_time', 0) for info in fog_tasks)
            ...
            avg_fog_wait = fog_wait / len(fog_tasks)
        ...
        avg_wait = total_wait / len(task_completion_info) if task_completion_info else 0
        ...
        fcfsc_logger.info("\nWait Time:")
        fcfsc_logger.info(f"  Total: {total_wait*1000:.6f}ms")
        fcfsc_logger.info(f"  Average: {avg_wait*1000:.6f}ms")
        fcfsc_logger.info("\nTotal Time:")
        ...
        if fog_tasks:
            ...
            fcfsc_logger.info("\nWait Time:")
            fcfsc_logger.info(f"  Total: {fog_wait*1000:.6f}ms")
            fcfsc_logger.info(f"  Average: {avg_fog_wait*1000:.6f}ms")
            fcfsc_logger.info("\nTotal Time:")
        ...
        if cloud_tasks:
            ...
            fcfsc_logger.info("\nPower Consumption:")
            fcfsc_logger.info(f"  Total Cloud Energy Consumed: {cloud_energy:.6f} Wh")
            fcfsc_logger.info(f"  Total Cloud Power Used: {cloud_power:.2f} W")
            fcfsc_logger.info(f"  Average Energy per Cloud Task: {avg_cloud_energy:.6f} Wh")
            fcfsc_logger.info(f"  Average Power per Cloud Task: {avg_cloud_power:.2f} W")
            fcfsc_logger.info("\nPer Node Statistics:")
            for node_name in cloud_node_workloads:
                ...
                fcfsc_logger.info(f"    Energy Consumed: {cloud_node_energy[node_name]:.6f} Wh")
                fcfsc_logger.info(f"    Power Used: {cloud_node_power[node_name]:.2f} W")
            fcfsc_logger.info("\nTransmission Time:")
            ...
            fcfsc_logger.info("\nWait Time:")
            fcfsc_logger.info(f"  Total: {cloud_wait*1000:.6f}ms")
            fcfsc_logger.info(f"  Average: {avg_cloud_wait*1000:.6f}ms")
            fcfsc_logger.info("\nTotal Time:")
